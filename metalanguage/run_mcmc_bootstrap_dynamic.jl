...
global original_base_semantics = deepcopy(base_semantics)
...
function compute_prior_probability(all_functions_with_sym)
    ...
    for function_sig in all_functions_with_sym
    ...
    global base_semantics = deepcopy(original_base_semantics)
...
function compute_function_subset_weight_scores(all_functions, mode="prior")
    min_possible_AST_size_dict = Dict(map(x -> format_new_function_string(x) => size(Meta.parse(generate_all_semantics(x, base_semantics, true)[1])), all_functions))
    weight_scores = Dict()
    for func in all_functions 
        # @show func 
...
function has_cycle(start_function_sig, start_definition)
    all_func_call_exprs = map(x -> "$(x.name)(", all_function_sigs)

    all_function_sigs_copy = deepcopy(all_function_sigs)
    start_name = start_function_sig.name

    start_function_sig_copy = filter(x -> x.name == start_name && length(x.arg_names) == length(start_function_sig.arg_names), all_function_sigs_copy)[1]
    start_function_sig_copy.definition = start_definition
    ...
    while length(queue) != 0 
        ...
        curr_func = filter(x -> x.name == curr_name, all_function_sigs_copy)[1]
        curr_definition = curr_func.definition 

        for call_expr in all_func_call_exprs 
            if occursin(call_expr, curr_definition)

                if call_expr == "$(start_name)("
                    return true
                end

                push!(queue, call_expr[1:end-1])
            end
        end
    end
    return false
...
function generate_all_semantics(function_sig, base_semantics, exclude_libs=false)
    ...
    # println("possible_semantics 0")
    # println(possible_semantics)

    if exclude_libs 
        lib_names = []
        for k in keys(base_semantics )
            # println(k)
            grammar_expr = base_semantics[k]
            # println(grammar_expr)
            parts = split(grammar_expr, "| lib_")[2:end]
            for part in parts 
                lib_name = split(part, " ")[1]
                push!(lib_names, lib_name)
            end
        end
        unique!(lib_names)
        for lib_name in lib_names 
            # println(lib_name)
            possible_semantics = filter(x -> !occursin("$(lib_name)(", x), possible_semantics)
        end
    end
    ...
    # println("possible_semantics")
    # println(possible_semantics)
    filter!(x -> !has_cycle(function_sig, x), possible_semantics)
    # println("possible_semantics 2")
    # println(possible_semantics)
...
function compute_likelihood(all_functions, test_config_names, repeats=1)
    ...
    function_definition_strs = map(x -> format_new_function_string(x, true), synthesized_funcs)
...
function proposal(current_state)
    ...
    if length(synthesized_old_functions) == length(old_functions)
        ...
        if first_choice == "edit"
            ...
            for sig in filter(x -> x != subset[1] && x.definition != "", old_functions)
                global base_semantics = update_semantics_cfg(base_semantics, sig)
            end
    ...
    if length(synthesized_old_functions) == length(old_functions)
        ...
        if length(synthesized_old_functions) == length(synthesized_new_functions)
            ...
            if !isnothing(changed_func_idx)
                ...
                for sig in filter(x -> x != old_functions[changed_func_idx] && x.definition != "", old_functions)
                    global base_semantics = update_semantics_cfg(base_semantics, sig)
                end
                ...
                for i in 1:length(all_functions) 
                    ...
                    for sig in filter(x -> x != all_functions[i] && x.definition != "", all_functions)
                        global base_semantics = update_semantics_cfg(base_semantics, sig)
                    end
    ...
    elseif length(synthesized_old_functions) == 0
        ...
        if length(synthesized_old_functions) == length(synthesized_new_functions)
            ...
            if !isnothing(edited_func_idx)
                ...
                for sig in filter(x -> x != old_functions[edited_func_idx] && x.definition != "", old_functions)
                    global base_semantics = update_semantics_cfg(base_semantics, sig)
                end
        ...
        elseif length(synthesized_old_functions) < length(synthesized_new_functions)
            ...
            for sig in filter(x -> x != old_functions[added_func_idx] && x.definition != "", old_functions)
                global base_semantics = update_semantics_cfg(base_semantics, sig)
            end
...
function run_mcmc(initial_state, test_config_names=[], iters=1000, repeats=1, intermediate_save_name="", test_name="", init=false)
    ...
    for i in 1:iters
        println("ITER $(i)")
        global all_function_sigs = deepcopy(current_state)
...
global all_function_sigs = [at_function, my_left_function_spot, left_of_function] # left_of_opposite_function
