...
function generate_all_semantics(function_sig, base_semantics, exclude_libs=false)
    ...
    # idx = findall(x -> x.name == function_sig.name, all_function_sigs)[1]

    # for i in idx:length(all_function_sigs)
    #     filter!(x -> !occursin("$(all_function_sigs[i].name)(", x), possible_semantics)
    # end
...
function compute_likelihood(all_functions, test_config_names, repeats=1)
    # update syntax
    synthesized_funcs = all_functions #filter(x -> x.definition != "", all_functions)
    ...
    total_score, scores, search_locations, temp_semantics = evaluate_semantics(nothing, "", 0, new_semantics_str, updated_syntax, 0, all_function_sigs, test_config_names, test_name, true)
...
function run_mcmc(initial_state, test_config_names=[], iters=1000, repeats=1, intermediate_save_name="", test_name="", init=false)
    println("old base_semantics")
    println(base_semantics)
    for function_sig in initial_state
